...
class JoueurShanon(Joueur):
    ...
    def joue(self, jeu):
        if self.serie in self.prec and self.prec[self.serie][1]: # Verifie si le comportement actuel est enregistre
            if self.condVic(self.prec[self.serie][0], 'c'):
                if self.derCoup == 'p':
                    res = 'f'
                else:
                    res = 'p'
    ...
    def clcl(self, coup, res):
        if self.derCoup != '':
            ...
            if self.serie in self.prec:
                if (coup == self.derCoup and self.prec[self.serie][0] == 'm') or \
                   (coup != self.derCoup and self.prec[self.serie][0] == 'c'):  # Si le comportement a deja ete rencontre
                    self.prec[self.serie][1] = True
                    # ~ print("comportement renforce", self.serie, self.prec[self.serie])
                    ...
                    # ~ print("Comportement mis à jour", self.serie, self.prec[self.serie])
                else:   # Si un autre comportement avait ete enregistre
                    self.prec[self.serie][0] = 'n'
                    self.prec[self.serie][1] = False
                    # ~ print("Comportement reinitailisé", self.serie, self.prec[self.serie])
